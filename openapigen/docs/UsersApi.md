...
| [**getViewedLastSentenceByMe**](#getviewedlastsentencebyme) | **GET** /users/me/viewedLastSentence/{titleId} | 小説の中で自分自身が最後に閲覧した投稿のIDを取得（認証あり） |
...
> SentenceIdOnly getViewedLastSentenceByMe()
...
const { status, data } = await apiInstance.getViewedLastSentenceByMe(titleId);
```
...
| Status code | Description                                          | Response headers |
| ----------- | ---------------------------------------------------- | ---------------- |
| **200**     | 小説の中で自分自身が最後に閲覧した投稿のID           | -                |
| **404**     | 小説の中で自分自身が最後に閲覧した投稿が見つからない | -                |
